...
- The composite accentedness score is only partly calibrated. Its phoneme path, the GOP flag and `normalize_gop`, is fitted against speechocean762. Its `fluency` path is now rebuilt on reference-free Whisper-timing features and fit against the same corpus's utterance fluency labels, replacing the old pause-ratio proxy that read 100 for nearly every clip, but the fit is GPU-bound and its linear coefficients ship as placeholders until it validates held-out, so fluency stays directional. Prosody stays directional on the same footing. Read the composite as directional until both fits validate.
