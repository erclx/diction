...
- `scripts/eye_contact_eval.py` is a fourth eval, for interview mode rather than pronunciation. It runs the MediaPipe face landmarker over the two interview clips, dumps per-frame yaw and pitch, and sweeps the yaw threshold to show where the looking-percentage separates a lens-look from an off-axis screen-glance, writing `data/eye_contact_eval.json` and `figures/eye_contact_separation.png`. It needs the `interview` extra and the gitignored clips under `tests/fixtures/interview/video/`, not speechocean762. The findings are in `backend/calibration/EYE_CONTACT_EVAL.md`.
...
- The interview eye-contact metric separates on head yaw, not pitch. Both axes distinguish the two clips, but the webcam sits above the monitor, so even a straight lens-look tilts the face up about 6 degrees, making an absolute pitch axis camera-dependent. Yaw stays honest because the lens is horizontally centered. The raw yaw cleanly separated the clips (good near 0, bad near minus 7) while the initial 15-degree threshold read a separation of zero, a lesson that a loose binary threshold can erase a real signal a distribution holds. The shipped 7-degree threshold is a two-clip directional placeholder, not a fit. See `EYE_CONTACT_EVAL.md`.
