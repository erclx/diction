...
- Interview CV, optional: `cd backend && uv sync --extra interview` for the MediaPipe posture and eye-contact scorer over PyAV. The pose and face model bundles self-download to `DICTION_MEDIAPIPE_CACHE_DIR` on first run, so no model file goes on disk up front. Skip the extra to run against the stub interview scorer with `DICTION_USE_STUB_INTERVIEW=true`.
...
`bun run dev:all` defaults to the stub scorer, explainer, critic, and synthesizer, so a fresh checkout boots without the `scoring`, `feedback`, and `tts` extras. Set `DICTION_DEV_MODELS=real` to load the installed models instead. Real mode forces every `DICTION_USE_STUB_*` flag off unless you export one yourself, so a stale stub flag left in `.env`, which `.env.example` ships defaulting to `true`, cannot silently serve canned scores under `dev:real`. In real mode `dev.sh` also provisions the runtime itself: it runs `uv sync --extra scoring --extra tts --extra feedback --extra interview` and fails fast with an actionable message when Ollama is unreachable. Kokoro self-downloads its model into the HuggingFace cache on the first real boot, so no voice provisioning step is needed and worktrees share that cache. This is what lets a fresh linked worktree run `dev:real`, since the branch carries neither the extras nor the cached model. A surface that skips the LLM, such as Shadowing, can still export `DICTION_USE_STUB_EXPLAINER=true` to bypass the Ollama requirement, since an explicit export wins over the real-mode default. The backend logs the resolved stack at startup as `model stack resolved: scorer=... explainer=...`, so a stubbed component shows in the boot log rather than passing for real. A standalone `cd backend && bun run dev` does not provision anything and still builds the real stack by default, so it needs the extras or the matching `DICTION_USE_STUB_*` flags, and startup fails with an actionable message for whichever stack is missing. The backend honors `DICTION_BACKEND_PORT` for its listen port.
