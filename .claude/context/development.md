...
`bun run dev:all` defaults to the stub `GopScorer`, `OllamaExplainer`, and `PiperSynthesizer`, so a fresh checkout boots without the `scoring`, `feedback`, and `tts` extras. Set `DICTION_DEV_MODELS=real` to load the installed models instead. In real mode `dev.sh` provisions the runtime itself: it runs `uv sync --extra scoring --extra tts --extra feedback`, downloads the Piper voice once to a machine-shared `$XDG_DATA_HOME/diction/voices` (override with `DICTION_VOICE_DIR`) so worktrees never re-download it, exports `DICTION_TTS_VOICE` to that path, and fails fast with an actionable message when Ollama is unreachable. This is what lets a fresh linked worktree run `dev:real`, since the branch carries neither the extras nor the gitignored voice. A surface that skips the LLM, such as Shadowing, can set `DICTION_USE_STUB_EXPLAINER=true` to bypass the Ollama requirement. A standalone `cd backend && bun run dev` does not provision anything and still builds the real stack by default, so it needs the extras, a voice at the default `backend/voices/` path, or the matching `DICTION_USE_STUB_*` flags, and startup fails with an actionable message for whichever stack is missing. The backend honors `DICTION_BACKEND_PORT` for its listen port.
