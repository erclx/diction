...
- The warm-up is wrapped in a try/except that logs a warning and continues, so a transient first-synthesis failure degrades to lazy first-click synthesis rather than aborting startup. Reference audio is a secondary enrichment path, and `.claude/rules/lib/360-model-runtime.md` holds that only the primary model may fail, so the scorer, history, and transcription stay up even when the warm-up cannot complete.
