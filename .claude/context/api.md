...
- The drill score route is the deliberate inverse of the passage route. It reuses the same scorer and the same `ClipTooWeakError` 422 boundary, but requests the lower `MIN_WORD_CLIP_SECONDS` floor so a single spoken word is not rejected as too short, and writes no session. It takes the target word plus the two contrast phonemes as form fields and calls `scorer.score_target_contrast`, which asks whether the competing phoneme scored higher than the target at the target's own frames rather than whether the target was acceptable on its own. The response keeps the passage-drill shape, `phoneme_quality` plus `flagged_phonemes`, where `flagged_phonemes` holds the target phoneme when it was substituted and is empty otherwise, so the frontend verdict logic is a plain membership check. The route never calls the LLM explainer, since a drill needs the verdict but not the plain-language reason. A single-word rep stays out of session history and the weak-sound rollup, which is why the route carries no `get_session` or `get_explainer` dependency.
