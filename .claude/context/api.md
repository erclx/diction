...
- `api/interview.py` owns `GET /api/interview/questions`, serving the operator's own interview questions parsed from the `DICTION_INTERVIEW_SOURCE_DIR` markdown bank. It has no model dependency and reads settings through `Depends(get_settings)`, parsing per request.
