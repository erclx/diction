...
- `api/drills.py` owns `POST /api/drills/minimal-pair/score`, scoring one drill word through the shared scorer and returning its phoneme-quality score, with no persistence.
...
- The drill score route is the deliberate inverse of the passage route. It reuses the same scorer and the same `ClipTooWeakError` 422 boundary, but requests the lower `MIN_WORD_CLIP_SECONDS` floor so a single spoken word is not rejected as too short, writes no session, and returns only the target word's `phoneme_quality` number. It shows that continuous score rather than a pass-or-retry verdict, because the flag threshold is an uncalibrated placeholder that misfires on vowel contrasts, so it never calls the LLM explainer or returns flagged words. A single-word rep stays out of session history and the weak-sound rollup, which is why the route carries no `get_session` or `get_explainer` dependency.
