...
- The app shell in `app.tsx` routes between surfaces with `react-router-dom`, so a refresh keeps the current surface and a session detail is deep-linkable. `BrowserRouter` mounts in `main.tsx` inside the `QueryClientProvider`. The routes are `/` for Practice, `/history` for the session list, `/history/:sessionId` for one session's detail, `/progress` for the progress dashboard, and `/drills/ear-training` for the ear-training drill, so `session-history.tsx` reads the selected id from the route param rather than local state. The nav uses `NavLink` inside a shadcn `SidebarMenuButton`, which drives `aria-current="page"` from the active route, and the active item takes a `sidebar-accent` fill through an `aria-[current=page]` variant rather than the primitive's own `data-active` prop. `react-router-dom` was chosen over TanStack Router to avoid a second routing-and-loader paradigm, and the router owns URL state only while TanStack Query keeps all fetching.
